import argparse
from src.common.loadData import load_data
...
def exec_model(training_set, test_set, use_cuda, model_dir, model_type, model_name, type_classify, features_stage, wandb_project=None, is_evaluate=False, is_sweeping=False, best_result_config=None):
    ...
    df_result = pd.DataFrame(y_predict, columns=['result'])
    df_result['result'] = df_result['result'].map({0: 'agree', 1: 'disagree', 2: 'discuss'})
    df_test = pd.concat([df_test, df_result], axis=0)
    df_test.to_csv('value.csv')
