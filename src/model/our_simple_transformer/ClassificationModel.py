...
from src.model.our_simple_transformer.classification_utils import (
    InputExample,
...
from src.model.roberta.RobertaForSequenceClassification import RobertaForSequenceClassification
...
class ClassificationModel():
    ...
    def evaluate(
        ...
        if isinstance(eval_df, str) and self.args.lazy_loading:
            ...
            elif "text_a" in eval_df.columns and "text_b" in eval_df.columns and "features" in eval_df.columns:
                eval_examples = [
                    InputExample(i, text_a, text_b, label, feature)
                    for i, (text_a, text_b, label, feature) in enumerate(
                        zip(eval_df["text_a"].astype(str), eval_df["text_b"].astype(str), eval_df["labels"], eval_df["features"])
                    )
                ]
